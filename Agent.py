...
Last update September 2016 HJMS'''
...
#MATCHING
    ...
    def vowel_match(self, v, w):
        #with lexical preservation
        #self.vowel_match_nh(v, w)

        #without lexical preservation
    ...
    def vowel_match_nh(self, v, w):
        ...
        for phone in rep:
            lm = (phone.length <= 200) == vl #both are short or both are long
            vpd = dist(phone, v)  #distance between phone and v
            if (vpd <= perc) and lm :
                if isa_baby: #pick the closest
                    ...
                    if ( ( (phone.weight > heaviest) or ( (not heaviest) and (vpd < closest) )) and
                          nh(phone, w) ):
                        best_phone = phone
    ...
    def no_homo(self, phone, w):
        ...
        w_nuc = w.nucleus
        phone_n = phone.name
        pot_w = Word.Word(w_ons, phone_n, w_cod)
    ...
    def vowel_match_rb(self, p):
        ...
        def lm(v):
            return ( (p.length > 200) == (v.length > 200) )
        ...
        def heaviest_match(vwls):
            neighbors = [v1 for v1 in vwls if ( (v1.weight > 0) and (p.euc(v1) <= perc) )]
            if not neighbors:
...
####WEIGHT WEIGHT
    ...
    def settle_conflicts(self):
        ...
        def ld(va, vb):
            length_diff = abs(va.length - vb.length)
